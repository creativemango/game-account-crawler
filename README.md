...
- **多源爬取**：螃蟹 HTTP API 直连；盼之 Playwright + webpack 拦截 + WASM 签名（patchright 反检测）
...
- **自动训练**：每日定时训练；冷启动时留空待回填（样本不足 `MIN_SAMPLES=200` 不训练）
...
│   └── pzds.py           # 盼之: patchright + webpack 拦截 + WASM 签名
├── parser/
│   └── wuwa.py           # 鸣潮解析: 统一 ParsedAccount 数据结构
├── valuer/
│   ├── features.py       # 28 维特征提取
...
依赖（见 [pyproject.toml](pyproject.toml)）：httpx, fastapi, uvicorn, pyyaml, numpy, patchright, lightgbm
...
| Worker | 作用 | 间隔 |
|---|---|---|
| `run_crawl_loop` | 爬取商品列表入库 | `crawl.interval_seconds`（默认 300s） |
| `run_detail_check_loop` | 轮询详情检测售出 | 600s |
| `run_valuer_loop` | 解析未估价账号 + 预测价值 | 300s |
| `run_train_loop` | 重新训练模型 | 86400s（每日） |
...
### 特征（28 维，见 [valuer/features.py](valuer/features.py)）
...
1. `train_loop` 每日触发，或 `POST /api/valuer/train` 手动触发
...
- `goodsPublic/page` 为 **WASM 签名接口**，无法纯算，必须浏览器内执行
- 方案：patchright 启动 Chromium → `Object.defineProperty(window, 'webpackJsonp', ...)` 拦截 webpack → 窃取 `__webpack_require__` → 取签名模块 → `generateSign(bodyStr, 'POST', timestamp, random)` → 页面 `fetch` 发请求（2.js 自动添加 `decode__1174` URL 参数）
- 浏览器实例按 `{game_id}:{platform}` 缓存复用（模块级 event loop + `atexit` 清理）
- 详情：详情页 SSR `window.__NUXT__.data[0].detailsData`，无需调 API
- 售出检测：详情页 HTML 含"已出售"标记
...
- 模型预测在数据量上来后会自动改善（当前 224 样本 P90 区间偏宽属正常）
