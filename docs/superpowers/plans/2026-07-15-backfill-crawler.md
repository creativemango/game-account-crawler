...
**Goal:** 提供两个独立 CLI 脚本（螃蟹/盼之），按 `--start-page` 向前翻页爬取历史在售账号数据，列表→详情→解析→特征→入库，不计算价值（交 main.py 的 valuer_loop 补全）。

**Architecture:** 新增 `backfill/` 包，含共享模块 `common.py`（`process_account` 函数）和两个源特定 CLI 脚本。对现有 `crawler/pxb7.py`、`crawler/pzds.py` 的 `crawl()` 加 `start_page` 可选参数（默认 1 兼容）。`db.py` 新增 `get_account_id` 按 (source, product_id) 查询 account_id。
...
| `backfill/pzds.py` | 新建 | 盼之向前爬虫 CLI |
...
### Task 3: crawler/pzds.py 的 crawl() 和 _crawl_async() 加 start_page 参数

**Files:**
- Modify: `crawler/pzds.py:393`（`_crawl_async` 签名 + 循环）
- Modify: `crawler/pzds.py:438`（`crawl` 签名 + 传参）
...
运行:
```powershell
uv run python -c "from crawler.pzds import crawl, _crawl_async; import inspect; sig1 = inspect.signature(crawl); sig2 = inspect.signature(_crawl_async); print('crawl start_page default:', sig1.parameters['start_page'].default); print('_crawl_async start_page default:', sig2.parameters['start_page'].default)"
```
Expected:
```
...
```powershell
git add crawler/pzds.py
git commit -m "feat(crawler/pzds): crawl() 和 _crawl_async() 加 start_page 参数"
```
...
### Task 6: backfill/pzds.py CLI

盼之向前爬虫脚本：复用浏览器实例 + 调 process_account。

**Files:**
- Create: `backfill/pzds.py`

- [ ] **Step 1: 创建 backfill/pzds.py**
...
运行:
```powershell
uv run python -m backfill.pzds --help
```
...
运行:
```powershell
uv run python -m backfill.pzds --game-id 303 --start-page 1 --max-pages 1
```
...
```
Expected: `pzds` 行的 `valued` 小于 `total`（或为 0）

- [ ] **Step 5: Commit**

```powershell
git add backfill/pzds.py
git commit -m "feat(backfill): 新增盼之向前爬虫 CLI"
```
...
Task 3 (pzds start_page)  ───┘                      └──→ Task 6 (pzds CLI)
```
...
- [ ] `python -m backfill.pzds --help` 正常
- [ ] 螃蟹爬 1 页：accounts + account_details 有数据，value 为 NULL
- [ ] 盼之爬 1 页：accounts + account_details 有数据，value 为 NULL
