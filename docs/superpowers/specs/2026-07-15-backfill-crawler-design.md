...
提供两个独立运行的 CLI 脚本，按源（螃蟹 / 螃蟹）向前翻页爬取历史在售账号数据。与 `main.py` 的定时爬虫（固定从第 1 页爬最新）互补，用于补充模型训练所需的存量数据。
...
1. 按 source 分发调详情接口
   ...
   - `[已移除]` → `crawler.[已移除]._get_client(game_id, platform)` 复用浏览器 + `client.fetch_goods_detail(goods_no)`
2. `parser.parse_pxb7` / `parser.parse_[已移除]` → `ParsedAccount.to_dict()`
...
CLI 脚本，负责螃蟹源特定的列表翻页 + 调 `process_account`。复用 `crawler.[已移除]._get_client` 的浏览器实例（跨页复用，`atexit` 自动清理）。
...
| 参数 | 必填 | 默认 | 说明 |
|---|---|---|---|
| `--game-id` | 是 | - | 游戏ID（螃蟹如 `10302`，螃蟹如 `303`） |
| `--start-page` | 是 | - | 起始页码 |
| `--max-pages` | 是 | - | 最多翻页数（防失控） |
| `--page-size` | 否 | 源默认 | 每页条数（螃蟹 16，螃蟹 10，与现有 crawl 一致） |
| `--platform` | 否 | `"6"` | 螃蟹专用，商品分类ID（`6`=成品号） |
...
# 螃蟹鸣潮：从第 4 页爬 50 页
uv run python -m backfill.[已移除] --game-id 303 --start-page 4 --max-pages 50 --platform 6
```
...
```
backfill.pxb7 / backfill.[已移除] (CLI, 单次运行)
  │
  ├─ 1. 列表翻页 (源特定)
  │     pxb7 → httpx 直连 selectSearchPageList
  │     [已移除] → [已移除]ApiClient.fetch_goods_page (浏览器+WASM签名)
  │
...
- **螃蟹浏览器崩溃**：清理 client 缓存，break 退出（下次运行重建）
...
backfill.common
  ...
  ├─ crawler.[已移除]._get_client / _get_loop (复用浏览器)
  ├─ parser.parse_pxb7 / parse_[已移除]
...
backfill.[已移除]
  ├─ crawler.[已移除].crawl (支持 start_page)
  └─ backfill.common.process_account
```
